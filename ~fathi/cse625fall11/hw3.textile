...
==|==F[~173~]==|== is finite and is an even number. It is finite because the number of DFAs that can be construct by __n__ states is finite and each DFA is equivalent to one and only one language. It is even because for any DFA with n states that accepts __L__, you can construct another DFA that accepts the compliment of __L__ by changing the accepting states to rejecting and visa versa.[1]
